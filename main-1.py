...
def main():
    ...
    # Set visualization
    # print("Visdom starting")
    # vis = visdom.Visdom()
    # text = vis.text("KR65535")
        ...
        for epoch in range(opts.start_epoch, opts.epochs + 1):
            ...
            if is_best:
                ...
                if IS_ON_NSML:
                    ...
                    torch.save(model.state_dict(), os.path.join('runs', opts.name + '_best.pt'))
                else:
                    torch.save(model.state_dict(), os.path.join('runs', opts.name + '_best.pt'))
            if (epoch + 1) % opts.save_epoch == 0:
                if IS_ON_NSML:
                    nsml.save(opts.name + '_e{}'.format(epoch))
                    torch.save(model.state_dict(), os.path.join('runs', opts.name + '_e{}.pt'.format(epoch)))
                else:
                    torch.save(model.state_dict(), os.path.join('runs', opts.name + '_e{}.pt'.format(epoch)))
...
def train(opts, train_loader, unlabel_loader, model, criterion, optimizer, epoch, use_gpu):
    ...
    for batch_idx in range(len(train_loader)):
        ...
        embed_u1, pred_x = model(inputs_x)
        ...
        loss_x, loss_un, weigts_mixing = criterion(pred_x, targets_x, pred_u2, targets_u, epoch+batch_idx/len(train_loader), opts.epochs)
